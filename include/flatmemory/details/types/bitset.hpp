...
public:
    ...
    public:
        ...
        [[nodiscard]] bool operator==(const const_iterator& other) const { return m_pos == other.m_pos; }
    ...
    static size_t get_index(size_t position) noexcept { return position / block_size; }

    static size_t get_offset(size_t position) noexcept { return position % block_size; }
};
...
public:
     ...
     */
    explicit View(uint8_t* buf) : m_buf(buf) { assert(m_buf); }
...
public:
    ...
    [[nodiscard]] size_t hash() const { return BitsetOperator::hash(*this); }
